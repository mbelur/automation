...
def enhance_input_model(input_model):
    ...
    Enhance the input model structure:
      ...
      - use references instead of attributes (to simplify dereference
      operations)
    ...
    def list_attr_to_map(element, attr_name, key='name'):
        if attr_name in element:
            element[attr_name] = OrderedDict([(item[key], item,)
                                              for item in element[attr_name]])
        else:
            element[attr_name] = OrderedDict()
        return element[attr_name]

    def link_element(element, attr_name, target_element,
                     target_key_attr='name'):
        element_map = element.setdefault(attr_name, {})
        element_map.setdefault(target_element[target_key_attr], target_element)

    def link_element_list(element, attr_name,
                          target_element_list, target_key_attr='name'):
        for target_element in target_element_list:
            link_element(element, attr_name, target_element, target_key_attr)

    def foreign_key_attr_to_ref(element, attr_name, target_map,
                                ref_list_attr=None, key_attr='name'):
        ...
        if isinstance(foreign_key, basestring) and foreign_key in target_map:
            ...
            if ref_list_attr:
                if key_attr:
                    target_map[foreign_key].setdefault(
                        ref_list_attr,
                        OrderedDict())[element[key_attr]] = element
                else:
                    target_map[foreign_key].setdefault(ref_list_attr,
                                                       []).append(element)

    def foreign_key_list_attr_to_ref_list(element, attr_name, target_map,
                                          ref_list_attr=None, key_attr='name'):
    ...
    for cp in input_model['control-planes'].itervalues():
        ...
        foreign_key_list_attr_to_ref_list(
    ...
    for interface_model in input_model['interface-models'].itervalues():
        ...
        for interface in interface_model['network-interfaces'].itervalues():
            ...
            foreign_key_list_attr_to_ref_list(
                ...
                ref_list_attr='network-interfaces')
            foreign_key_list_attr_to_ref_list(
                interface, 'forced-network-groups',
                input_model['network-groups'],
                ref_list_attr='network-interfaces')
    ...
    def add_neutron_network_tags(network_group_name, tags):
        ...
        foreign_key_attr_to_ref(
            ...
            key_attr=None)  # Use a null key_attr value
    ...
    if neutron_config_data:
        ...
        external_networks = list_attr_to_map(
            neutron_config_data['data'],
            'neutron_external_networks')
        provider_networks = list_attr_to_map(
            neutron_config_data['data'],
    ...
    for network_group in input_model['network-groups'].itervalues():
        ...
        #    network_group, ref_list_attr='network-groups')
        foreign_key_list_attr_to_ref_list(
    ...
    input_model['networks'] = \
        OrderedDict(filter(
            lambda net: net[1]['network-group']
                        in input_model['network-groups'],
    ...
    for server_group in input_model['server-groups'].itervalues():
        foreign_key_list_attr_to_ref_list(
            server_group, 'networks',
            input_model['networks'], ref_list_attr='server-groups')
        foreign_key_list_attr_to_ref_list(
            server_group, 'server-groups',
            input_model['server-groups'],
            ref_list_attr='server-group-parents')

    # Delete server groups that aren't referenced
    # by servers or other server groups
    prune_unused_items(
        input_model['server-groups'],
        ['servers', 'server-group-parents'])
...
def generate_heat_model(input_model, virt_config):
    ...
    for interface_model in input_model['interface-models'].itervalues():
        ...
        # configuration
        heat_interface_model['ports'] = [port[1] for _, port in enumerate(
    ...
    for server in input_model['servers'].itervalues():
        ...
        heat_server = dict(
            ...
            image=images.get(server['id'],
                             images.get(server['role']['name'],
                             images['default']))[
                server.get('distro-id', virt_config['default_distro_id'])],
            flavor=flavors.get(server['id'],
                               flavors.get(server['role']['name'])),
            is_admin=False,
            is_controller=False,
            is_compute=False
        )
        # Figure out which server is the CLM host, which are controllers
        # and which are computes
        for service_group in server['role'].get('clusters', {}).values() + \
                             server['role'].get('resources', {}).values():
            ...
            # a controller node
                ...
                if filter(lambda sc: sc not in
                                     virt_config['clm_service_components'],
                          service_group['service-components']):
                    heat_server['is_controller'] = True
                    if not heat_server['flavor']:
                        heat_server['flavor'] = \
                            flavors['default_controller_flavor']
